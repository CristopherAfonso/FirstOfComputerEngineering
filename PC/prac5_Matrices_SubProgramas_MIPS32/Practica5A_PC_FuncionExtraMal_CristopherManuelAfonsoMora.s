...
#}
#
#void InvertColumn(std::vector<int>& matrix,
...
#                InvertColumn(Matrix, nrows, ncols, line1);
#            }
...
ColumnaInvertir:        .asciiz     "¿Columna a invertir? "
FinPrograma:            .asciiz     "\n\nFin Del Programa\n"
Piola:                  .asciiz     "\n\nPiola\n\n"
...
# $s6 = std::vector<int> aux_vec1 // En las funciones RowsSwap, ColumnSwap y InvertColumn
...
#
InvertColumn:
    addi $sp,$sp,-28            #Hago espacio en la pila para 7 variables

    sw $a3,24($sp)              #Valor line1, aquí será col1
    sw $a2,20($sp)              #Dirección nrows
    sw $a1,16($sp)              #Dirección de ncols
    sw $a0,12($sp)              #Dirección base original de la matriz
    sw $s2,8($sp)               #Variable "i" que usarán los dos for
    sw $s1,4($sp)               #Dirección de la matriz ubicada en la posición matrix[col1]
    sw $s0,0($sp)               #Variable aux_vec1
                                #El registro $t3 será int j{0}
    

    la $s0,aux_vec1             # Cargo la dirección de memoria de aux_vec1

    move $s1,$a0                # Cargo la dirección base de matrix en $s1

    lw $t1,16($sp)              
    lw $t1,0($t1)               # Cargo desde la pila, el valor de col1

    mul $t1,$t1,4               # Ahora tengo la cantidad de bytes que debo desplazarme hasta col1

    addu $s1,$s1,$t1            # Guardo en $s1 la posición matrix[col1]

    move $s2,$zero              # Hago int i{0}
    For9:
        lw $t0,20($sp)
        lw $t0,0($t0)           # Cargo desde la pila, el valor de nrows
        bge $s2,$t0,FinFor9     # Comparo i < nrows

        mul $t1,$t0,$s2         # Hago nrows * i, resultado en $t1
        mul $t1,$t1,4           # Ahora tengo la cantidad de bytes que debo desplazarme
        addu $t1,$t1,$s1        # Ahora tengo la posición en memoria matrix[col1 + nrows*i] en $t1
    ...
    lw $t1,16($sp)              # Cargo desde la pila, el valor de col1
    lw $t1,0($t1)
    ...
    lw $s4,20($sp)
    lw $s4,0($s4)              # Cargo en $s4, el valor de nrows
    addi $s4,$s4,-1            # Hago int i{nrows - 1}
    move $t3,$zero             # Hago int j{0}
    For10:

        blt $s4,$t0,FinFor10   # Comparo i >= 0

        lw $t0,20($sp)         # Cargo en $t0, la dirección de nrows
        lw $t0,0($t0)          # Sobreescirbo $t0, con el valor de nrows

        mul $t1,$t0,$t3        # Hago nrows * j, resultado en $t1
        mul $t1,$t1,4          # Ahora tengo la cantidad de bytes que debo desplazarme
        addu $t1,$t1,$s1       # Ahora tengo la posición en memoria matrix[col1 + nrows*j] en $t1

        lw $t2,0($s0)          # Cargo en $t2, el primer elemento no sacado ya de aux_vec1
        sw $t2,0($t1)          # Meto en matrix[col1 + nrows*i], el valor de $t2
...
#        }
    While2:
        ...
        sw $v0,0($s3)                   #Reescribo la variable operation_code
            ...
            While3:
                li $t0,-1               #Constante -1, actuará de variable en la función booleana al no haber un operador "seq" en MIPS
                ...
                slt $t1,$t2,$t1         #Comparo operation_code > 3 , resultado en $t1
...
#            ShowMatrix(Matrix, nrows, ncols);
            If8:
                lw $t0,0($s3)                       #Cargo operation_code
                li $t1,3                            #Cargo constante 3
                bne $t0,$t1,FinIf8                  #Si operation_code != 3, no ejecuta If8

                DoWhile3:
                    la $a0,ColumnaInvertir
                    li $v0,4                        #Le digo al sistema que muestre un string
                    syscall
...
#                    if((line1 < 0) || (line1 > (ncols-1))) {                     
                        slt $t0,$s4,$zero           #Comparo line1 < 0
                        lw $t4,ncols                #Cargo ncols
                        addi $t4,$t4,-1             #Obtengo ncols -1
                        sgt $t1,$s4,$t4             #Comparo line1 > (ncols-1)

                        or $t0,$t0,$t1              #Comparo los dos valores del If9
                        not $t0,$t0                 #Si hay un error en los parámetros, $t0 tendrá un -2, si todo va bien, habrá un -1
                        addi $t0,$t0,1              #Ahora, si hay un error, habrá un -1, y si todo va bien, habrá un 0
                    ...
                    slt $t0,$s4,$zero               #Aquí hago lo mismo que en el If9
                    lw $t4,ncols            
                    addi $t4,$t4,-1         
                    sgt $t1,$s4,$t4         
                ...
                move $a3,$s4
                la $a2,nrows
                la $a1,ncols                        #Le pasamos a la función los 4 valores que necesita
                la $a0,matrix
                jal InvertColumn                    #Como todos los parámetros de la función se pasan por referencia, no cargo ninguna variable en los registros $a para darle a la matriz
