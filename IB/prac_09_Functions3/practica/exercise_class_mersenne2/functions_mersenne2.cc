...
void AllNumbersOddOrEven(int& odd, int& even) {
	std::cout << std::endl;
	std::cout << "De todos los números que te acabo de mostrar, " << odd << " son números impares y, " << even << " son números pares" << std::endl;
}
...
void AllMersenneNumbers(int& user_number) {
    ...
    for(const auto actual_vector_value: all_n_first_prime_numbers) {	   
	    ...
	    if(actual_possible_mersenne_number % kTwo == 0) {
		    ++even;
            }
	    else {
		    ++odd;
