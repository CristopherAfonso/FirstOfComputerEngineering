...
void Introduction() {
    ...
    std::cout << "Cuando tenga los 3 números, devolveré por pantalla la media, la desviación estándar, el máximo y el mínimo de todos los números que hayan el el vector" << std::endl;
    std::cout << "Que todos los números que tenga el vector, serán de tipo double y habrán sido creados en el intervalo que los dos últimos números que me des denoten" << std::endl;
...
double StandardDeviation(double& actual_value, double& average) {
    return pow(actual_value - average, 2.0);
}
...
void FinalMessage2(double& average, double& standard_deviation, double& maximum, double& minimum) {

    std::cout << std::endl << std::endl;
    std::cout << "La media de todos esos datos es: " << average << std::endl;
    std::cout << "La desviación estandar de todos esos datos es: " << standard_deviation << std::endl;
