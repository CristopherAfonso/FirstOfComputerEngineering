...
{ 
  ...
  if (M.solve(exit_way, alternative_way))
  {
    cout << "¡¡ Se ha encontrado una salida estandar al laberinto !!" << endl;
    cout << M << endl << endl;
  ...
  alternative_way = false;
  if (M.solve(exit_way2, alternative_way))
  {
    cout << "¡¡ Se ha encontrado una salida alternativa al laberinto !!" << endl;
    cout << M << endl << endl;
    
    dll_node_t<pair_t<int>>* aux = exit_way2.get_head();
    for(int i{1}; i <= M.get_m_maze(); ++i)
    {
      for(int j{1}; j <= M.get_n_maze(); ++j)
      {
        aux = exit_way2.get_head();
        while(aux != exit_way2.get_tail())
        {
          if((i == aux->get_data().get_val()) && (j == aux->get_data().get_inx()))
          {
            cout << PATH_CHR;
  ...
  {
    cout << "No se ha podido encontrar la salida alternativa del laberinto..." << endl;
  }
