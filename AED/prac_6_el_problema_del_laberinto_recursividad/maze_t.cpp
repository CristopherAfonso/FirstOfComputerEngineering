...
bool
maze_t::solve(dll_t<pair_t<int>>& exit_way, bool alternative_way) {
  ...
  if(alternative_way == true)
  {
    return solve_(i_start_, j_start_, exit_way);
  }
  else 
  {
    return solve2_(i_start_, j_start_, exit_way);
  }
...
{ 
  ...
  if((i <= matrix_.get_m()) && (i > 0) && (j <= matrix_.get_n()) && (j > 0) &&
...
maze_t::solve_(const int i, const int j, dll_t<pair_t<int>>& exit_way)
{
  ...
  if(is_ok_(i + i_d[0], j + j_d[0]) == true) { // Dirección Norte
    if(solve_(i + i_d[0], j + j_d[0], exit_way)) {
      matrix_.at(i + i_d[0], j + j_d[0]) = PATH_ID;
      pair_t<int>* pointer = new pair_t<int>(i + i_d[0], j + j_d[0]);
      exit_way.push_front(new dll_node_t<pair_t<int>>(*(pointer)));
      return true;
    }
  }

  if(is_ok_(i + i_d[1], j + j_d[1]) == true) { // Dirección Este
    if(solve_(i + i_d[1], j + j_d[1], exit_way)) {
      matrix_.at(i + i_d[1], j + j_d[1]) = PATH_ID;
      pair_t<int>* pointer = new pair_t<int>(i + i_d[1], j + j_d[1]);
      exit_way.push_front(new dll_node_t<pair_t<int>>(*(pointer)));
      return true;
    }
  }

  if(is_ok_(i + i_d[2], j + j_d[2]) == true) { // Dirección Sur
    if(solve_(i + i_d[2], j + j_d[2], exit_way)) {
      matrix_.at(i + i_d[2], j + j_d[2]) = PATH_ID;
      pair_t<int>* pointer = new pair_t<int>(i + i_d[2], j + j_d[2]);
      exit_way.push_front(new dll_node_t<pair_t<int>>(*(pointer)));
      return true;
    }
  }

  if(is_ok_(i + i_d[3], j + j_d[3]) == true) { // Dirección Oeste
    if(solve_(i + i_d[3], j + j_d[3], exit_way)) {
      matrix_.at(i + i_d[3], j + j_d[3]) = PATH_ID;
      pair_t<int>* pointer = new pair_t<int>(i + i_d[3], j + j_d[3]);
      exit_way.push_front(new dll_node_t<pair_t<int>>(*(pointer)));
      return true;
    }
  }
...
bool
maze_t::solve2_(const int i, const int j, dll_t<pair_t<int>>& exit_way)
{
  // CASO BASE:
  // retornar 'true' si 'i' y 'j' han llegado a la salida
